...
def VIGP_Unlinked(n_iter,
    ...
    for iter in tqdm(range(n_iter)):
        ...
        resid_sq += mu_lambda_beta ** 2 * torch.trace(X @ (V_X_star - M_X_star.T @ M_X_star) @ X.T)
        resid_sq += sigmasq_lambda_beta * torch.trace(X @ V_X_star @ X.T)
        resid_sq += torch.trace(mu_W @ (V_S_star - M_S_star.T @ M_S_star) @ mu_W.T)
        resid_sq += torch.trace(torch.block_diag(*[V_S_star] * n_blocks) @ Sigma_W)          

        # TODO: check this (lambda_a2 / lambda_b2) or (lambda_b2 / lambda_a2)
